...
Ensure that a document accepted into the vector base actually contains the text it claims to, by detecting extraction that is incomplete (not only absent or encoding-broken), judging quality across the whole document rather than per isolated page, escalating to the existing rescue cascade (pdfplumber → Tesseract → GPT-4o Vision) when needed, failing visibly when no method recovers adequate quality, recording the method and quality that justified acceptance, and bounding the cost of per-page optical extraction.
