#!/usr/bin/env bash
# Roda UMA VEZ na EC2 PÚBLICA do frontend (Nginx + Next.js), recém-criada, a
# partir deste mesmo diretório `deploy/` (copiado pra EC2 via scp/rsync — não
# precisa do resto do repositório, mas precisa de `nginx/nginx.conf` junto).
# Pré-requisitos antes de rodar (ver deploy/README.md):
#   - Instance profile IAM anexado à instância, com permissão de leitura no
#     repositório ECR do frontend (não usa chave estática — autentica via
#     metadata da instância).
#   - `deploy/.env` criado a partir de `deploy/.env.prod.example`, preenchido.
#   - `deploy/nginx/nginx.conf` presente (copiado junto com este diretório).
#
# Uso: sudo ./ec2-bootstrap-frontend.sh
...
for f in .env docker-compose.frontend.yml nginx/nginx.conf; do
    if [ ! -f "$f" ]; then
...
echo "==> Pull das imagens (nginx:alpine + frontend do ECR)"
...
echo "  docker compose -f docker-compose.frontend.yml --env-file .env ps"
